...
| **25** | **Release Artifact Filename** | `TESTED` | `ToolRecapV3-v3.0.2-windows-portable.zip` generated in `release/`. | Verified by packaging script. |
| **26** | **Release Version** | `TESTED` | Version `3.0.2` defined in `toolrecap_v3/__version__.py` and verified in `package_marker.json`. | Verified in `--selfcheck` output. |
| **27** | **SHA256 Checksum** | `TESTED` | Current checksum is recorded in `release/ToolRecapV3-v3.0.2-windows-portable.zip.sha256.txt` and verified via `verify_checksum`. | Verified by automated packaging tests. |
...
The packaging script `build_portable.py` bundles the following files into `dist/ToolRecapV3/` and `release/ToolRecapV3-v3.0.2-windows-portable.zip`:
